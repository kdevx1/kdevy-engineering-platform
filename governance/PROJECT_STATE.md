# Estado do Projeto

## Identificação

- Projeto: KdevX Engineering Platform
- Sigla: KEP
- Proprietária: KdevX Systems
- Classificação padrão: Interno
- Versão inicial: 0.1.0 Foundation
- Onda atual: Onda 1 - Foundation
- Corporate Identity: Incubada na KEP
- Repository Boundaries Review: concluída
- Dívida arquitetural: extração futura de `docs/corporate/` para `kdevx-systems`
- Data de criação: 2026-07-06

## Estado atual

A KEP está em sua fundação inicial. Esta versão estabelece a estrutura mínima do repositório proprietário de engenharia da KdevX Systems.

## Escopo da versão 0.1.0 Foundation

- Estrutura inicial do repositório.
- Documentação-base da plataforma-mãe.
- Políticas iniciais de propriedade intelectual e classificação da informação.
- Registro inicial de decisão arquitetural.
- Roadmap por ondas.
- Templates de trabalho e revisão.

## Fora do escopo

- Código-fonte do AutoBody ERP.
- Implementações de produto.
- Scripts operacionais definitivos.
- Integrações automatizadas.
- Publicação externa de documentação.

## Relação com produtos

O AutoBody ERP é o primeiro produto que consumirá a KEP. Ele deve permanecer em repositório separado.

## Critérios de saúde

- A estrutura base existe.
- A documentação está em português do Brasil.
- A KEP está explicitamente definida como plataforma-mãe.
- A KEKB está definida como módulo de conhecimento da KEP.
- As políticas proprietárias iniciais estão registradas.
